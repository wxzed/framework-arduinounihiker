...
void TFT_eSPI::pushImageDMA(int32_t x, int32_t y, int32_t w, int32_t h, uint16_t const* image)
{
  ...
  memset(&trans, 0, sizeof(spi_transaction_t));
  while(len>0x4000){
    trans.user = NULL;//(void *)1;
    trans.tx_buffer = buffer;   //Data pointer
    trans.length = 0x4000 * 16;   //Data length, in bits
    trans.flags = 0;           //SPI_TRANS_USE_TXDATA flag
    spi_device_acquire_bus(dmaHAL,portMAX_DELAY);//占用SPI通道直到完成
    ret = spi_device_queue_trans(dmaHAL, &trans, portMAX_DELAY);//发送任务
    assert(ret == ESP_OK);
    ret = spi_device_get_trans_result(dmaHAL, &rtrans, portMAX_DELAY);//等待DMA完成
    assert(ret == ESP_OK);
    spi_device_release_bus(dmaHAL);
    len -= 0x4000; buffer+= 0x4000; // Arbitrarily send 1K pixel blocks (2Kbytes)
    //spiBusyCheck++;
  }
...
bool TFT_eSPI::initDMA(bool ctrl_cs)
{
  ...
  spi_device_interface_config_t devcfg = {
    ...
    .queue_size = 20,            // Not using queues
